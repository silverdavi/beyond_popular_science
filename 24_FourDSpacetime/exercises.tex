...
\begin{enumerate}
    ...
    \textit{(Why $c$ rather than $a$ or $b$? By similarity, the shape depends on $\phi$ and a squared length. Any side could be used, but expressing the result in terms of $c$ emphasises the relation to the hypotenuse.)}
    \begin{center}
...
\end{enumerate}
\vspace{0.5em}
Dimensional analysis can be rigorously formalised using a graded algebraic structure where physical quantities lie in one-dimensional vector spaces $V^d$ indexed by a group $G$ of exponents (e.g., $\mathbb{R}^n$ for base units). Multiplication corresponds to the tensor product $V^a \otimes V^b \cong V^{a+b}$, and one only adds elements within the same space $V^d$. For example, a quantity in kilograms lies in $V^{(1,0,0,\ldots)}$ and one in metres per second lies in $V^{(0,1,-1,\ldots)}$, reflecting mass and velocity dimensions respectively. Physical laws hold under any consistent rescaling of units, leading to constraints like Buckingham's $\pi$-theorem.  
