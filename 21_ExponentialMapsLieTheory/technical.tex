...
\newcommand{\axlabel}[1]{\makebox[1.2em][l]{(#1)}}
...
\vspace{-0.25em}
\begin{itemize}[leftmargin=2.5em,labelwidth=1.5em,labelsep=0.5em,topsep=0pt,itemsep=0pt]
  \item[\axlabel{A}] Additivity: \( K(g \OpB{+} h) = K(g) \OpB{+} K(h) \).
  \item[\axlabel{L}] Leibniz: \( K(g \OpB{\cdot} h) = K(g) \OpB{\cdot} h \OpB{+} g \OpB{\cdot} K(h) \).
  \item[\axlabel{C}] Kills Constants: \( K(c_{\B{b}}) = c_{\OpB{0}} \) where \( c_{\B{b}}(\A{x}) \equiv \B{b} \).
  \item[\axlabel{T}] Translation Invariance: \( K \circ T_{\A{y}} = T_{\A{y}} \circ K \).
\end{itemize}
...
\vspace{-0.25em}
\begin{itemize}[leftmargin=2.5em,labelwidth=1.5em,labelsep=0.5em,topsep=0pt,itemsep=0pt]
  \item[\axlabel{E}] Eigenfunction: \( K(f) = \B{\lambda} \OpB{\cdot} f \).
  \item[\axlabel{N}] Normalisation: \( f(\OpA{0}) = \OpB{1} \).
  \item[\axlabel{U}] Uniqueness: Evaluation at \( \OpA{0} \) is injective on \( \ker(K - \B{\lambda} I) \).
\end{itemize}
...
\textit{Intuition}:
Translation invariance makes $K$ commute with shifts: $K(T_{\A{y}} f) = T_{\A{y}} K(f)$. Thus every translation $T_{\A{y}} f$ of an eigenfunction remains in the $\lambda$-eigenspace of $K$. By (U), translation acts on this eigenspace by scalar multiplication: $T_{\A{y}}f = c_{f(\A{y})} \OpB{\cdot} f$, meaning that translating $f$ by $\A{y}$ yields the function $\A{x} \mapsto f(\A{y}) \OpB{\cdot} f(\A{x})$. Commuting with the derivation therefore forces addition in the domain (the group operation generating translations) to correspond to multiplication in the codomain (the scalar action on the eigenspace).
