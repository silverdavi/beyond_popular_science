...
The molecular orbitals (MOs) of conjugated systems can be described as linear combinations of atomic $\mathrm{p}$ orbitals. For a linear polyene with \( n \) $\mathrm{p}$ orbitals, the \( k^\text{th} \) MO has the form \(\Psi_k = \sum_{j=1}^n \sin(\pi k j/(n+1))\, \mathrm{p}_j\), where \( \mathrm{p}_j \) are orthogonal atomic orbitals. The phase of the terminal lobes in the HOMO (highest occupied MO) determines the allowed mode of bond formation. For example:
\begin{itemize}
