...
\]
where \( \Lambda \) is the von Mangoldt function and \( w(n) \) is a smooth function supported on \( n \in [x, 2x] \). The weights emphasise values where multiple \( n + h_i \) are likely prime:
\[
...
Zhang proved that primes remain equidistributed up to moduli \( q \le x^\theta \) for \( \theta > 1/2 \), surpassing the Bombieri–Vinogradov barrier (\( \theta = 1/2 \)).
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Maynard's Variant:} Lowered \( N \) further, generalised to \( m \) primes in bounded intervals.
