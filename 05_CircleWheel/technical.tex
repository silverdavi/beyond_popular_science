...
Proto-Indo-European (PIE) contained labiovelar stops (\piefont{*kʷ}, \piefont{*gʷ}, \piefont{*gʷʰ}) with simultaneous velar closure and labialization, contrasting with plain velars (\piefont{*k}, \piefont{*g}, \piefont{*gʰ}) and palatalized velars (\piefont{*ḱ}, \piefont{*ǵ}, \piefont{*ǵʰ}). The PIE root \piefont{*kʷékʷlos}, a reduplicated form of \piefont{*kʷel-} (\QSOpen{}to turn\QSClose{}), underwent systematic shifts across branches.

\textbf{In Greek},
By Mycenaean Greek (c. 1400 BCE, attested in Linear B, the earliest known form of the Greek writing system), labiovelars were still distinguished. Later Greek neutralised them context-dependently: \piefont{*kʷ} became \piefont{t} before front vowels (\textgreek{πέντε} \QSOpen{}five\QSClose{} < PIE \piefont{*pénkʷe}), \piefont{p} in many environments (\textgreek{λείπω} \QSOpen{}I leave\QSClose{} < PIE \piefont{*leikʷ-}), and \piefont{k} in certain contexts:
\[
\textgreek{κύκλος} \text{ (\emph{kyklos}) < PIE } \piefont{*kʷékʷlos}.
\]
The labiovelars were thus fully neutralised, with reflexes depending on phonological environment.
...
\textbf{In Latin}, \piefont{*kʷ} was generally preserved as \emph{qu} in most environments (\emph{quis}, \emph{quo}, \emph{equus}, \emph{aqua}, \emph{quattuor}). However, certain roots show regular de-labialization to \emph{c-}, notably the \piefont{*kʷel-} family: PIE \piefont{*kʷel-} → Latin \emph{colere} (\QSOpen{}to cultivate\QSClose{}), \emph{incola} (\QSOpen{}inhabitant\QSClose{}). This reflects dissimilation: the labial element of \piefont{*kʷ} is lost before a rounded vowel in the following syllable.
In contrast, \emph{circulus} (\QSOpen{}circle\QSClose{}), from Greek \emph{kirkos} (\QSOpen{}ring\QSClose{}), derives from PIE \piefont{*(s)ker-} \QSOpen{}to turn, bend\QSClose{}—a distinct root without labiovelars.
...
Online Etymology Dictionary: \textcolor{black}{\url{https://www.etymonline.com/}}\\
}
\end{technical}
