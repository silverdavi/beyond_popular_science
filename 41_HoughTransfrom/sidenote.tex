\begin{SideNotePage}{
  ...
  Two roads connect cities A and B without crossing. A pair of cars, tied by a rope of length < $2R$, can travel side by side on these roads from A to B. Now consider two circular wagons of radius $R$, each centred on its own path—one going A to B, the other B to A. Let $(x, y)$ track their positions along the two roads, forming a trajectory in the unit square $I = \{(x, y) : 0 \leq x, y \leq 1\}$. The cars’ path goes from $(0, 0)$ to $(1, 1)$; the wagons' from $(1, 0)$ to $(0, 1)$. By topology, these curves must intersect. At the intersection, the wagons occupy the same positions as the cars once did, yet their centres are < $2R$ apart, and each wagon has radius $R$, so a collision is unavoidable. A fact that is difficult to prove directly. (After Konstantinov, in Arnold's \textit{ODEs}.) \par    
